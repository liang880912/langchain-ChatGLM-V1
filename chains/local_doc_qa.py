...
class LocalDocQA:
    ...
    top_k: int = VECTOR_SEARCH_TOP_K ###从知识库中最高返回几个文本块
    chunk_size: int = CHUNK_SIZE
    chunk_conent: bool = True
    score_threshold: int = VECTOR_SEARCH_SCORE_THRESHOLD ###检索内容匹配值分数，0-1000，500以下时匹配结果较好

    def init_cfg(self,
                 embedding_model: str = EMBEDDING_MODEL, ###word2vec模型
        ...
        self.embeddings = HuggingFaceEmbeddings(model_name=embedding_model_dict[embedding_model],
                                                model_kwargs={'device': embedding_device}) ###从huggingface加载模型
        self.top_k = top_k

    def init_knowledge_vector_store(self,
                                    filepath: str or List[str],
                                    vs_path: str or os.PathLike = None,
                                    sentence_size=SENTENCE_SIZE):###文章切片大小，句子长度要合适
        loaded_files = []
        failed_files = []
        if isinstance(filepath, str): 
            if not os.path.exists(filepath):
                print("路径不存在")
                return None
            elif os.path.isfile(filepath): ###本地知识库路径是一个文件
                file = os.path.split(filepath)[-1]
                try: ###docs列表中装的是切片后的一段段话，
                    docs = load_file(filepath, sentence_size) ###可以加载txt\pdf\png等各种形式文件
                    ...
                    return None
            elif os.path.isdir(filepath): ###本地知识库路径是一个文件夹，遍历每一个文件
                docs = []
